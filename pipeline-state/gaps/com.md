...
- docsEscape: the Flutter Calls "Join Session" quick-start documents NO login prerequisite, yet joinSession fails with ERROR_AUTH_TOKEN ("User auth token is null") until the Calls SDK is separately logged in
    Confirmed live android + iOS: the outgoing call screen now presents (Fix C), but `CometChatCalls.joinSession(sessionId:…)` errors "User auth token is null" on BOTH platforms. Two documentation/SDK inconsistencies:
    (1) The official Join Session page (/calls/flutter/join-session) "Join with Session ID" quick-start says only "Pass a session ID and the SDK automatically generates the token and joins the call" — it never tells you the Calls SDK must be logged in first. But its own error table lists ERROR_AUTH_TOKEN = "User not logged in or auth token invalid", which is exactly what fires. The prerequisite `CometChatCalls.loginWithAuthToken(...)` before joinSession is absent from the join recipe.
    (2) The natural token source for that login, `CometChat.getUserAuthToken()`, returns NULL after a UI-Kit `CometChatUIKit.loginWithAuthToken(...)` login — so the documented getter cannot supply the token the Calls login needs. Working fix (matches Deskline's `_capturedAuthToken`): capture the auth token from your OWN backend login response at app-login time and retain it, then pass THAT to `CometChatCalls.loginWithAuthToken` before joinSession; `getUserAuthToken()` is only a fallback. Ask: cometchat-flutter-v6-calls must (a) document the `CometChatCalls.loginWithAuthToken` prerequisite in the join-session flow, and (b) note that `CometChat.getUserAuthToken()` is unreliable (returns null) after `loginWithAuthToken`, so apps must retain the auth token themselves.
