...
**/build/
# Maven/Java build output. NOT covered by **/build/ (that's gradle) — a Spring backend puts its
# compiled classes AND a fat jar in target/, so without this the baseline tracks ~75 .class files
# plus a ~52MB jar, which then gets PUSHED to the public use-case repo (GitHub warns >50MB).
target/
**/target/
.gradle/
android/.gradle/
...
def augment_path(S):
    """Make self-installed tools (maestro, android) findable by later stages, and pin DEVELOPER_DIR to an
    Xcode whose simulator SDK actually has an installed runtime. The DEVELOPER_DIR pin is set here (one
    hook every stage already calls) so build, integrate, demo AND verify all shell out to the SAME usable
    Xcode — otherwise a stray `xcode-select` pointing at an Xcode with no matching runtime fails every
    iOS build with 'Found no destinations' (exit 70) before a line is compiled."""
    ...
    dev = mobile.select_developer_dir()
    if dev and dev != os.environ.get("DEVELOPER_DIR"):
        os.environ["DEVELOPER_DIR"] = dev
        print(f"  [toolchain] DEVELOPER_DIR -> {dev}")
    return _sh
...
def stage_demo(S, uc):
    ...
    login_failed = []
    # A platform that LAUNCHED but whose app id could not be resolved silently SKIPPED the login gate
    # below — the gate that exists to catch "launches but can't sign in". Surface it as a coverage
    # failure instead of passing quietly (it read as a green demo on fin until vision caught iOS).
    login_unarmed = [p for p in ("android", "ios")
                     if shots.get(p, {}).get("ok") and not plat_app_id.get(p)]
    if login_unarmed:
        die_gate(f"demo:{uc['slug']} mobile LOGIN gate UNARMED for {login_unarmed} — the app launched but "
                 f"its package/bundle id did not resolve (providers.resolve_app_id), so sign-in was never "
                 f"verified. Refusing to report a green demo on an unverified client. tag=harness")
